class Board(object):
    ...
    def _check_overlap(self, wd_pos, word, direct, dictionary):
        ...
        for char_point, character in zip(wd_pos, word):
            if self.board[char_point[0]][char_point[1]] != '.' and self.board[char_point[0]][char_point[1]] != character:
                raise ValueError('Overlaped character is wrong')
            if self.board[char_point[0]][char_point[1]] != '.' and self.board[char_point[0]][char_point[1]] == character:
                is_ol = True
            for nm in neighbor_mask:
                if self.board[char_point[0]+nm[0]][char_point[1]+nm[1]] != '.':
    ...
    def _get_neighbor_word(self, int_char, int_point, move_mask):
        ...
        for mm in move_mask:
            ...
            while 0 <= char_point[0] < self.size and 0 <= char_point[1] < self.size and self.board[char_point[0]][char_point[1]] != '.':
    ...
    def add_word(self, posit, direct, word, dictionary):
        ...
        for char_point, character in zip(wd_pos, word):
            if self.board[char_point[0]][char_point[1]] == '.':
